...
def mask_invalid(data):
    # bit-wise for fmask in hls S30
    cloud_pos=1;     shadow_pos=3;    water_pos=5; adj_pos=2
...
def extract_doy(filename):
    tmpvar=filename.split('/')[-1].split('.')[3].split('T')[0]
    return int(tmpvar)

def keep_float(num, dp):
    return format(num, "."+str(dp)+"f")
...
nth_epoch=model_path.split('/')[-1].split('_')[-2]

model = load_model(model_path,custom_objects={'f1_m': f1_m})

N_BANDS=13

inputdir='path to test dataset'
...
def call_code(filename):
    ...
    fmask_data=hls_data[-1];    fmask=mask_invalid(fmask_data)
    ...
    #to_be_predict[to_be_predict<0]=-2000
